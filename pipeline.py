...
class PipelineRunner(object):
    ...
    def add(self, processor):
        if not isinstance(processor, PipelineProcessor):
            raise Exception('Processor should not be an isinstance of PipelineProcessor')
    ...
    def run(self):
        ...
        if(self.context['frame_number'] % 50 == 0 ):
...
class ContourDetection(PipelineProcessor):
    ...
    def filter_mask(self, img, a=None):
        ...
        #dilate image to merge adjacent blobs
    ...
    def detect_vehicles(self, fg_mask, context):
        ...
        for(i, contour) in enumerate(contours):
            ...
            #Set a boolean if the dimensions meet some minimum
...
class VehicleCounter(PipelineProcessor):
    # Counts vehicles that entered the zone of detection
    ...
    def __call__(self, context):
        ...
        if not self.paths:
            ...
            #link new points with old paths
            new_paths = []
            ...
            for path in self.paths:
                ...
                for p in points:
                    if len(path) == 1:
                        ...
                        #predict next point based on last 2
                ...
                if all(_match) and _min <= self.max_dst:
            ...
            if len(points):
                for p in points:
                    #dont add points that have already been counted
        ...
        for i, path in enumerate(self.paths):
            ...
            if( len(d) >= 2 and not self.check_exit(d[0][1]) and self.check_exit(d[1][1]) and self.path_size<=len(path)):
